...
void Scene_Zelda::sMovement()
{
    ...
    Vec2 playerVelocity(0, 0);
         ...
         if (m_player->getComponent<CInput>().up) { playerVelocity = Vec2(0, -m_playerConfig.SPEED); }
    else if (m_player->getComponent<CInput>().down) { playerVelocity = Vec2(0, m_playerConfig.SPEED); }
    else if (m_player->getComponent<CInput>().left) { playerVelocity = Vec2(-m_playerConfig.SPEED, 0); }
    else if (m_player->getComponent<CInput>().right) { playerVelocity = Vec2(m_playerConfig.SPEED, 0); }
...
void Scene_Zelda::sCamera()
{
    ...
    else          //Room camera
    {
        ...
        if (cPos.x < 0) { xRoom = int(cPos.x / 1280)-1 * 1280; }
        else            { xRoom = int(cPos.x / 1280) * 1280; }
        
        if (cPos.y < 0) { yRoom = int(cPos.y / 768)-1 * 768; }
        else            { yRoom = int(cPos.y / 768) * 768; }
